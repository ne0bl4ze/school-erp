...
function StatCard({ value, label, color }) {
  const colors = {
    orange: '#DF740C', green: '#22C55E',
    red: '#EF4444', blue: '#3B82F6', teal: '#14B8A6',
  };
  const c = colors[color];
  return (
    <div
      style={{ background:'#181818', border:'1px solid #2C2C2C', borderRadius:10, padding:'18px 14px 14px', textAlign:'center', cursor:'pointer', position:'relative', overflow:'hidden', transition:'transform 0.15s' }}
    ...
    <div className="page fade-up">
      ...
      <div className="card" style={{ marginBottom:18 }}>
        <div className="card-header">
          ...
          <div className="card-actions">
            <Link to="/timetable" style={{ fontSize:11.5, fontWeight:500, color:'#DF740C' }}>Full Timetable →</Link>
          </div>
        </div>
        {todaySlots.length === 0 ? (
          <div className="empty-state">No periods scheduled for today</div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.min(todaySlots.length, 5)},1fr)` }}>
            {todaySlots.map((slot, i) => (
              <div key={i} style={{ padding:'16px 18px', borderRight:'1px solid #2C2C2C', transition:'background 0.12s' }}
              ...
              >
                ...
                <div style={{ fontSize:12.5, fontWeight:600, color:'#fff', marginBottom:5, lineHeight:1.35 }}>
                  {slot.course?.name}
                </div>
                <span style={{ fontSize:10.5, color:'#606060', padding:'2px 7px', background:'#222', borderRadius:4 }}>
                  {slot.room}
                </span>
              </div>
            ))}
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div className="card-actions">
            <Link to="/announcements" style={{ fontSize:11.5, fontWeight:500, color:'#DF740C' }}>View All →</Link>
          </div>
        </div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                ...
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:'#fff' }}>
                    {ann.author?.name} <span style={{ fontWeight:400, color:'#606060' }}>posted a notice</span>
                  </div>
                  <div style={{ fontSize:10.5, color:'#606060', marginTop:1 }}>{new Date(ann.createdAt).toLocaleString()}</div>
                </div>
